...
Як це працює: у віддану розмітку контакт потрапляє **перевернутим і без
`href`** (`moc.atadove@olleh`), а CSS `unicode-bidi: bidi-override` розвертає
його назад для ока — тому людина бачить правильний напис навіть із вимкненим
JavaScript. Після гідратації `ContactLink` збирає справжній текст і робить
його посиланням `mailto:` / `tel:`, тож копіювання, клік і скрінрідери
працюють як завжди.

Що це дає: у HTML немає ні `mailto:`, ні рядка, схожого на адресу, ні номера —
збирачі адрес, які просто читають розмітку, не отримують нічого. Проти
скрапера, який виконує JavaScript, це не захищає (таких меншість).

Два правила, щоб не зламати:
