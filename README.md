...
- `src` порожній (типово) — **на головній секція не рендериться взагалі**
  (постер у макеті — стокова заглушка); на решті сторінок показується сам постер;
- `src` заданий — секція повертається на головну, постер стає кнопкою,
...
7. **Вкладки на What We Do** (секція `9325:1161`) — під'єднані. З 6 підписів у макеті є
